...
class TicketInfoViewModelTest : BaseTest() {
    ...
    fun setup() {
        ...
        viewModel = TicketInfoViewModel(
            ...
            facade = BaseTicketInfoFacade(
                ticketInfoMapper = TicketInfoResultMapper(),
                columnUiMapper = TestColumnUiMapper()
            ),
    ...
    fun test_collect_ticket() {
        ...
        viewModel.fetchTicket(
            ticketUi = TicketUi(
                ...
                assignedMemberNames = listOf("test assignee member name"),
        ...
        repository.makeExpectedTicketInfoResult(
            TicketInfoResult.Success(
                ticket = Ticket(
                    ...
                    assignedMemberNames = listOf("success assignee member name"),
                    column = Column.InProgress,
                    creationDate = java.time.LocalDateTime.of(
                        2024,
                        2,
                    ...
                    assignedMemberIds = listOf("success assigned member id")
                )
            )
        )
        assertEquals(1, repository.ticketCalledList.size)
    ...
    private class TestColumnUiMapper : ColumnUi.Mapper<Column> {
